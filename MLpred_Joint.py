...
## MLPred_Joint: Disease risk prediction based on ensemble machine learning model on genotype and L&E data
...
# B. Individual's lifestyle and Environment factors (L&E). It should be a plain text file and each row is a L&E factor and each column represents an individual.
...
parser = ArgumentParser(description="MLPred_Joint is an ensemble learning model to predict disease using genetic markers and L&E information",add_help=False)
...
parser.add_argument('--LE_train','-p1', type=str,help="Text file with L&E information of training set.", default='None')
parser.add_argument('--LE_valid','-p2', type=str,help="Text file with L&E information of validation set.", default='None')
parser.add_argument('--LE_test','-p3', type=str,help="Text file with L&E information of test set.",  default='None')
...
def LD_pruning(args):
    ...
    if os.stat(args.outpath+"/association_analysis/5E4_SNP").st_size!=0:
        ...
        if len(open(args.outpath+"/association_analysis/5E4_SNP",'rU').readlines())!=1:
            ...
            os.system('plink --bfile '+args.outpath+'/LD_pruning/5E3_train_extract --prune --recode A --out '+args.outpath+'/LD_pruning/train_5E4_recodeA')
    if os.stat(args.outpath+"/association_analysis/5E5_SNP").st_size!=0:
        ...
        if len(open(args.outpath+"/association_analysis/5E5_SNP",'rU').readlines())!=1:
            ...
            os.system('plink --bfile '+args.outpath+'/LD_pruning/5E3_train_extract --prune --recode A --out '+args.outpath+'/LD_pruning/train_5E5_recodeA')
    ...
    if os.stat(args.outpath+"/association_analysis/5E6_SNP").st_size!=0:
        ...
        if len(open(args.outpath+"/association_analysis/5E6_SNP",'rU').readlines())!=1:
            ...
            os.system('plink --bfile '+args.outpath+'/LD_pruning/5E3_train_extract --prune --recode A --out '+args.outpath+'/LD_pruning/train_5E6_recodeA')
...
def transfer_plain_npy(args):
    ...
    for data in ["train","valid","test"]:
        if data=="train":
            self_phenotype=np.genfromtxt(args.LE_train,dtype=float)
        elif data=='valid':
            self_phenotype=np.genfromtxt(args.LE_valid,dtype=float)
        else:
            self_phenotype=np.genfromtxt(args.LE_test,dtype=float)
...
def ML_model_LE(args):
    if not os.path.exists(args.outpath+'/ML_LE'):
        os.system('mkdir '+args.outpath+'/ML_LE')
    disease_train=np.load(args.outpath+'/train/disease_train.npy')
    Cxlist_NN=[(20,20,20),(30,30),(10,10,10,10),(30,20,10)]
    Cxlist_ada=[DecisionTreeClassifier(),LogisticRegression(),ExtraTreeClassifier(),GaussianNB()]
    Cxlist_GB=[0.0001,0.001,0.01,0.1]
    ...
    train_pheno=np.load(args.outpath+'/train/phenotype_train.npy').transpose()
    valid_pheno=np.load(args.outpath+'/valid/phenotype_valid.npy').transpose()
    test_pheno=np.load(args.outpath+'/test/phenotype_test.npy').transpose()
    
    for t in [1,2,3,4]:
        Cx_NN=Cxlist_NN[t-1]
        NN=MLPClassifier(hidden_layer_sizes=Cx_NN,max_iter=1000)
        NN.fit(train_pheno,disease_train)
        Y=NN.predict_proba(valid_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_NN_valid.npy',Y[:,1])
        Y=NN.predict_proba(test_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_NN_test.npy',Y[:,1])

        Cx_ada=Cxlist_ada[t-1]
        ada = AdaBoostClassifier(base_estimator=Cx_ada)
        ada.fit(train_pheno,disease_train)
        Y=ada.predict_proba(valid_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_ada_valid.npy',Y[:,1])
        Y=ada.predict_proba(test_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_ada_test.npy',Y[:,1])

        Cx_GB=Cxlist_GB[t-1]
        GB = GradientBoostingClassifier(min_impurity_decrease=Cx_GB)
        GB.fit(train_pheno,disease_train)
        Y=GB.predict_proba(valid_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_GB_valid.npy',Y[:,1])
        Y=GB.predict_proba(test_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_GB_test.npy',Y[:,1])

        Cx_LR=Cxlist_LR[t-1]
        LR = LogisticRegression(penalty='l1', C=Cx_LR, max_iter=10000)
        LR.fit(train_pheno,disease_train)
        Y=LR.predict_proba(valid_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_LR_valid.npy',Y[:,1])
        Y=LR.predict_proba(test_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_LR_test.npy',Y[:,1])

        Cx_RF=Cxlist_RF[t-1]
        RF = RandomForestClassifier(min_impurity_decrease=Cx_RF)
        RF.fit(train_pheno,disease_train)
        Y=RF.predict_proba(valid_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_RF_valid.npy',Y[:,1])
        Y=RF.predict_proba(test_pheno)
        np.save(args.outpath+'/ML_LE/'+str(t)+'_RF_test.npy',Y[:,1])


def ensemble_model_Genotype(args):
    ...
    out2.close()
    np.save(args.outpath+'/Ensemble_Geno/Risk_geno_valid.npy',Y1[:,1])
    np.save(args.outpath+'/Ensemble_Geno/Risk_geno_test.npy',Y2[:,1])
...
def ensemble_model_LE(args):
    disease_valid=np.load(args.outpath+'/valid/disease_valid.npy')
    disease_test=np.load(args.outpath+'/test/disease_test.npy')
    for i in ['GB','NN','ada','RF','LR']:
        if i=='GB':
            filename1=args.outpath+'/ML_LE/1_'+i+'_valid.npy'
            filename2=args.outpath+'/ML_LE/1_'+i+'_test.npy'
            data1=np.load(filename1)
            data2=np.load(filename2)
            enseinput=data1.reshape(-1,1)
            ensetest=data2.reshape(-1,1)
            for n in ['1','2','3','4']:
                filename1=args.outpath+'/ML_LE/'+n+'_'+i+'_valid.npy'
                filename2=args.outpath+'/ML_LE/'+n+'_'+i+'_test.npy'
                data1=np.load(filename1)
                data2=np.load(filename2)
                if i=='GB' and n=='1':
                    continue
    ...
    Y1=LR.predict_proba(enseinput)
    Y2=LR.predict_proba(ensetest)
    if not os.path.exists(args.outpath+'/Ensemble_LE'):
        os.system('mkdir '+args.outpath+'/Ensemble_LE')
    out1=open(args.outpath+'/Ensemble_LE/Risk_LE_valid.scores','w')
    out2=open(args.outpath+'/Ensemble_LE/Risk_LE_test.scores','w')
    for x in Y1[:,1]:
        out1.write(str(round(x,3))+'\n')
    out1.close()
    for x in Y2[:,1]:
        out2.write(str(round(x,3))+'\n')
    out2.close()
    np.save(args.outpath+'/Ensemble_LE/Risk_LE_valid.npy',Y1[:,1])
    np.save(args.outpath+'/Ensemble_LE/Risk_LE_test.npy',Y2[:,1])

    out=open(args.outpath+'/Ensemble_LE/AUC_Ensemble.scores','w')
    AUC_valid=roc_auc_score(disease_valid,Y1[:,1])
    AUC_test=roc_auc_score(disease_test,Y2[:,1])
    out.write('AUC score for valid set '+str(round(AUC_valid,3))+'\n')
    out.write('AUC score for test set '+str(round(AUC_test,3))+'\n')
...
def ensemble_model_all(args):
    disease_valid=np.load(args.outpath+'/valid/disease_valid.npy')
    disease_test=np.load(args.outpath+'/test/disease_test.npy')
    genotype_valid=np.load(args.outpath+'/Ensemble_Geno/Risk_geno_valid.npy')
    phenotype_valid=np.load(args.outpath+'/Ensemble_LE/Risk_LE_valid.npy')
    genotype_test=np.load(args.outpath+'/Ensemble_Geno/Risk_geno_test.npy')
    phenotype_test=np.load(args.outpath+'/Ensemble_LE/Risk_LE_test.npy')
    inputdata1=np.concatenate((genotype_valid.reshape(-1,1),phenotype_valid.reshape(-1,1)),axis=1)
    inputdata2=np.concatenate((genotype_test.reshape(-1,1),phenotype_test.reshape(-1,1)),axis=1)
    LR = LogisticRegression(penalty='l1', max_iter=10000,C=10000)
    LR.fit(inputdata1,disease_valid)
    Y1=LR.predict_proba(inputdata1)
    Y2=LR.predict_proba(inputdata2)
    if not os.path.exists(args.outpath+'/Ensemble_All'):
        os.system('mkdir '+args.outpath+'/Ensemble_All')
    out1=open(args.outpath+'/Ensemble_All/Risk_All_valid.scores','w')
    out2=open(args.outpath+'/Ensemble_All/Risk_All_test.scores','w')
    for x in Y1[:,1]:
        out1.write(str(round(x,3))+'\n')
    out1.close()
    for x in Y2[:,1]:
        out2.write(str(round(x,3))+'\n')
    out2.close()
    np.save(args.outpath+'/Ensemble_All/Risk_All_valid.npy',Y1[:,1])
    np.save(args.outpath+'/Ensemble_All/Risk_All_test.npy',Y2[:,1])
    out=open(args.outpath+'/Ensemble_All/AUC_Ensemble.scores','w')
    AUC_valid=roc_auc_score(disease_valid,Y1[:,1])
    AUC_test=roc_auc_score(disease_test,Y2[:,1])
    out.write('AUC score for valid set '+str(round(AUC_valid,3))+'\n')
    out.write('AUC score for test set '+str(round(AUC_test,3))+'\n')
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        sys.stdout.write("Please use 'python MLpred_Joint.py -h' for more information!\n")
    elif args.geno_train=="None" or args.geno_valid=="None" or args.geno_test=="None" or args.LE_train=="None" or args.LE_valid=="None" or args.LE_test=="None":
        ...
        if args.LE_train=="None":
            print('Please provide L&E file of training set!\n')
        if args.LE_valid=="None":
            print('Please provide L&E file of validation set!\n')
        if args.LE_test=="None":
            print('Please provide L&E file of test set!\n')
    else:
        valid=1
        ...
        if not os.path.exists(args.geno_test+'.bed'):
            ...
            valid=0
        if not os.path.exists(args.LE_train):
            print('L&E file of training set does not exist!\n')
            valid=0
        if not os.path.exists(args.LE_valid):
            print('L&E file of validation set does not exist!\n')
            valid=0
        if not os.path.exists(args.LE_test):
            print('L&E file of test set does not exist!\n')
            valid=0
        ...
        if valid==1:
            ...
            if step3_finish==1:
               ...
               print('Step4: Recode genotype and L&E into .npy files')
            ...
            if not os.path.exists(args.outpath+'/ML_Geno/1_1_ada_test.npy') or not os.path.exists(args.outpath+'/ML_Geno/1_1_GB_test.npy') or not os.path.exists(args.outpath+'/ML_Geno/1_1_LR_test.npy') or not os.path.exists(args.outpath+'/ML_Geno/1_1_NN_test.npy') or not os.path.exists(args.outpath+'/ML_Geno/1_1_RF_test.npy'):
                step5_finish=0
                print('Failed to generate *.npy for individual ML model prediction on genotype data in Step5')
            if not os.path.exists(args.outpath+'/ML_LE/1_ada_test.npy') or not os.path.exists(args.outpath+'/ML_LE/1_GB_test.npy') or not os.path.exists(args.outpath+'/ML_LE/1_LR_test.npy') or not os.path.exists(args.outpath+'/ML_LE/1_NN_test.npy') or not os.path.exists(args.outpath+'/ML_LE/1_RF_test.npy'):
                step5_finish=0
                print('Failed to generate *.npy for individual ML model prediction on L&E factors in Step5')
            if step5_finish==1:
                print('Step5 finished\n')
                print('Step 6: Calculate personal risk scores and AUC scores by ensemble learning')
                ensemble_model_Genotype(args)
                ensemble_model_LE(args)
                ensemble_model_all(args)
